"""
Main entry point for the environment sensor firmware.
Runs on the Arduino Nano RP2040 Connect with an SSD1306 OLED display and
DHT20 temperature/humidity sensor connected to the I2C bus.
...
Boot sequence:
  ...
  6. Continue to main application logic: sensor → display → MQTT publish loop.
...
Required libraries in /lib/:
  ...
  adafruit_ahtx0.mpy      (DHT20 sensor)
  adafruit_ssd1306.mpy    (OLED display)
  adafruit_framebuf.mpy   (SSD1306 dependency)
...
# WIFI_PASSWORD and MQTT_USER / MQTT_PASSWORD are intentionally absent —
# open networks and anonymous broker access use empty values.
_REQUIRED_SETTINGS = (
    "WIFI_SSID",
    "SERVER_URL",
    "DEVICE_API_KEY",
...
_POLL_INTERVAL_DEFAULT = 30  # seconds between sensor readings
...
# Hardware-derived unique device identifier — stable across reboots and OTA.
...
def _update_current_version(version):
    ...
    content = (
        ...
        f'MQTT_BROKER = "{_escape_toml(os.getenv("MQTT_BROKER", ""))}"\n'
        f'MQTT_PORT = {int(os.getenv("MQTT_PORT") or 1883)}\n'
        f'MQTT_USER = "{_escape_toml(os.getenv("MQTT_USER", ""))}"\n'
        f'MQTT_PASSWORD = "{_escape_toml(os.getenv("MQTT_PASSWORD", ""))}"\n'
        f'DEVICE_NAME = "{_escape_toml(os.getenv("DEVICE_NAME", ""))}"\n'
        f'POLL_INTERVAL = {int(os.getenv("POLL_INTERVAL") or _POLL_INTERVAL_DEFAULT)}\n'
    )
...
def _init_i2c_devices():
    """Initialise the I2C bus, sensor, and display. Returns (i2c, sensor_device, oled).

    Extracted so the sensor loop can call it again to recover from a bus lockup.
    If sensor or display initialisation fails after the bus is created, the bus
    is deinited before re-raising so the pins are freed for the next attempt.
    """
    i2c = busio.I2C(board.SCL, board.SDA)
    try:
        sensor_device = sensor.setup(i2c)
        ...
        try:
            ...
            pass
        raise
    return i2c, sensor_device, oled


def _run_app(session, pool, device_name, poll_interval):
    """Initialise hardware and run the sensor → display → MQTT publish loop.

    Parameters
    ----------
    session : adafruit_requests.Session or None
        Active HTTP session if WiFi connected; None if offline. When None,
        sensor data is shown on the display but MQTT publishing is skipped.
    pool : socketpool.SocketPool or None
        Socket pool for MQTT; None when offline.
    device_name : str
        Human-readable label for this device in Home Assistant.
    poll_interval : int
    ...
    if pool is not None:
        ...
        try:
            ...
            mqtt_ha.publish_discovery(mqtt_client, DEVICE_ID, device_name)
        except Exception as e:
            print("[code] MQTT setup failed:", e)
            mqtt_client = None

    # sensor_device / oled start as None; the loop initialises them on the
    # first iteration and re-initialises them after any I2C failure.
    i2c = sensor_device = oled = None

    print("[code] Entering sensor loop. Poll interval:", poll_interval, "s")
    while True:
        # ── Ensure I2C devices are available ──────────────────────────────────
        if sensor_device is None or oled is None:
            try:
                i2c, sensor_device, oled = _init_i2c_devices()
            except Exception as e:
                print("[code] I2C init failed, retrying next cycle:", e)
                time.sleep(poll_interval)
                continue

        # ── Sensor → display → MQTT ───────────────────────────────────────────
        try:
            temp, humidity = sensor.read(sensor_device)
            display.show(oled, temp, humidity)
            if mqtt_client is not None:
                mqtt_ha.publish_readings(mqtt_client, DEVICE_ID, temp, humidity)
        except OSError as e:
            # I2C bus lockup: a failed transaction can leave the bus stuck.
            # Deinit, clear the device references, and let the next cycle
            # attempt a fresh initialisation.
            print("[code] I2C error, reinitialising bus:", e)
            try:
                i2c.deinit()
            except Exception:
                pass
            ...
            print("[code] Sensor loop error:", e)
...
def main():
    ...
    poll_interval = int(os.getenv("POLL_INTERVAL") or _POLL_INTERVAL_DEFAULT)

    print("[code] Booting. Version:", current_version)
    print("[code] Device ID:", DEVICE_ID)
    print("[code] Device name:", device_name)
    ...
    if not _connect_wifi(esp, ssid, password):
        print("[code] Proceeding without network.")
        _run_app(session=None, pool=None, device_name=device_name, poll_interval=poll_interval)
        return
    ...
    print("[code] Boot complete. Starting application.")
    _run_app(session=session, pool=pool, device_name=device_name, poll_interval=poll_interval)
